...
public class FromBottomRight {
	...
	public FromBottomRight(BinaryMatrix binaryMatrix, LargestArea[][] result) {
		super();
			...
			for (int j = 0; j < binaryMatrix.getCol(); j++) {
				...
				if (binaryMatrix.getMatrix()[i][j]) {
					...
					if (max == area_tmp[0]) {
						...
						updateResultTop(i, j, temp2[i][j], temp3[i][j], max);
					} else {
						updateResultTop(i, j, temp4[i][j], temp1[i][j], max);
					}
	...
	private void updateResultTop(int i, int j, int width, int height, int area) {
		// Compute the coordinates of the top left corner of that rectangle !
		int newI = i - height + 1;
		int newJ = j - width + 1;
		updateResult(newI, newJ, width, height, area);
	}
